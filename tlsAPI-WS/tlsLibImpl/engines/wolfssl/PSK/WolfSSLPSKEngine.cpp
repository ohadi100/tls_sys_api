...
WolfSSLPSKEngine::WolfSSLToEngineError()
{
    ...
    FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". wolfssl error code: ", err, ", error message: ", buffer, ".");
...
WolfSSLPSKEngine::DoSSLHandshake()
{
    ...
    if (WOLFSSL_SUCCESS != wolfSSL_use_psk_identity_hint(m_ssl.get(), m_keys.hint.c_str())) {
        ...
        FND_LOG_ERROR(GetLogger(), "wolfSSL_use_psk_identity_hint failed: connectionName: ", m_connectionLoggingName.c_str(), ", wolfssl error code: ", res, ", error message: ", buffer, ".");
        return RC_TLS_ENGINE_FATAL_ERROR;
    }
    ...
    if (WOLFSSL_SUCCESS != wolfSSL_set_cipher_list(m_ssl.get(), ciphers.c_str())) {
        ...
        char buffer[80];
        wolfSSL_ERR_error_string(res, buffer); 
        FND_LOG_ERROR(GetLogger(), "wolfSSL_set_cipher_list failed: connectionName: ", m_connectionLoggingName.c_str(), ", wolfssl error code: ", res, ", error message: ", buffer, ".");
        return RC_TLS_ENGINE_FATAL_ERROR;
    }
    ...
    if ((WOLFSSL_SUCCESS !=
        ...
        char buffer[80];
        wolfSSL_ERR_error_string(res, buffer); 
        FND_LOG_ERROR(GetLogger(), "wolfSSL_set_ex_data failed: connectionName: ", m_connectionLoggingName.c_str(), ", wolfssl error code: ", res, ", error message: ", buffer, ".");
    ...
    if (m_isServer) {
        ...
        if (WOLFSSL_SUCCESS != res) {
            FND_LOG_ERROR(GetLogger(), "wolfSSL_accept failed.");
            return WolfSSLToEngineError();
        }
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". wolfssl server handshake finished");
    } else {
        res = wolfSSL_connect(m_ssl.get());
        if (WOLFSSL_SUCCESS != res) {
            FND_LOG_ERROR(GetLogger(), "wolfSSL_connect failed.");
            return WolfSSLToEngineError();
        }
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". wolfssl client handshake finished");
    }
...
WolfSSLPSKEngine::Shutdown()
{
    ...
    {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". wolfSSL_shutdown returned ", res);
    }
