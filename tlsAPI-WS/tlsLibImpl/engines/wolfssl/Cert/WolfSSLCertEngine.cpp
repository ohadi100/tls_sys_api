...
WolfSSLCertEngine::WolfSSLToEngineError()
{
    ...
    FND_LOG_ERROR(GetLogger(), "connectionName: ",
                m_connectionLoggingName.c_str(), ". wolfssl error code: ", err, ", error message: ", buffer, "." );
...
WolfSSLCertEngine::setCipherSuitesListUseCase(TLSCipherSuiteUseCasesSettings const& cipherSuiteSettings)
{
    ...
    case CSUSLegacy:
        FND_LOG_DEBUG(GetLogger(), "CSUSLegacy cipher suite ids list");
    ...
    case CSUSLongtermSecure:
        FND_LOG_DEBUG(GetLogger(), "CSUSLongtermSecure cipher suite ids list");
    ...
    case CSUSIanaRecommended:
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". CSUSIanaRecommended cipher suite ids list");
        m_validCiphers       = ianaRecommendedCipherSuites;
        m_cipherSuiteUseCase = CSUSIanaRecommended;
        break;

    case CSUSDefaultWithSoftFail:
        FND_LOG_DEBUG(GetLogger(), "CSUSDefaultWithSoftFail cipher suite list");
    ...
    case CSUSDefault:
    default:
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". CSUSDefault cipher suite ids list");
        m_validCiphers       = defaultCipherSuites;
                                             ...
                                             std::vector<char>&                certHashKey) const
{
    ...
    if (0 != wc_Sha256Hash(
        ...
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to calculate hash");
        return false;
    }
    ...
    if (0 != Base64_Encode_NoNl(shaSum, sizeof(shaSum), reinterpret_cast<byte*>(certHashKey.data()), &outLen)) {
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to calculate base64");
        return false;
    }
...
WolfSSLCertEngine::ctxInit()
{
    ...
    if (!m_ctx) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". wolfSSL_CTX_new failed.");
    ...
    if (wolfSSL_CTX_SetMinVersion(m_ctx.get(), WOLFSSL_TLSV1_2) != WOLFSSL_SUCCESS) {
        m_ctx = nullptr;
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". wolfSSL_CTX_SetMinVersion failed.");
        return WolfSSLToEngineError();
    }
    ...
    if (serverCert.empty()) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Can't find trust store: ", m_certStoreId.c_str(), ".");
    ...
    if (WOLFSSL_SUCCESS != res) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Error loading CA certs from buffer");
        return WolfSSLToEngineError();
    }
...
WolfSSLCertEngine::sslInit()
{
    ...
    if ((wolfSSL_UseSupportedCurve(m_ssl.get(), WOLFSSL_ECC_SECP256R1)) != WOLFSSL_SUCCESS ||
        ...
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". use wolfSSL_UseSupportedCurve failed");
        return WolfSSLToEngineError();
    }

    std::string alpn_list;
    if (getAlpnProtocol(alpn_list)) {
        FND_LOG_VERBOSE(GetLogger(), "connectionName: ",  m_connectionLoggingName.c_str(), ". ALPN is used.");
        ...
        if (WOLFSSL_SUCCESS !=
            ...
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". wolfSSL_UseALPN failed");
            return WolfSSLToEngineError();
        }
    } else {
        FND_LOG_VERBOSE(GetLogger(), "connectionName: ",  m_connectionLoggingName.c_str(), ". ALPN is unused.");
    }

    if ((m_hostName.size() > 0) &&
        (wolfSSL_UseSNI(m_ssl.get(), 0, m_hostName.c_str(), (word16)m_hostName.size()) != WOLFSSL_SUCCESS)) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". use SNI failed");
        return WolfSSLToEngineError();
    }

    if (wolfSSL_set_cipher_list(m_ssl.get(), m_validCiphers.c_str()) != WOLFSSL_SUCCESS) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Fatal error: can't set cipher list");
        return WolfSSLToEngineError();
    }

    if (wolfSSL_set_ex_data(m_ssl.get(), WolfSSLCertEngine::EX_DATA_IDX_WOLFSSL_CERT_ENGINE_OBJ, (void*)(this)) !=
        WOLFSSL_SUCCESS) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to set engine object in ssl context ex data");
        return WolfSSLToEngineError();
    }
...
WolfSSLCertEngine::teeInit()
{
    if (!m_clientCertificateSetID.empty()) {
        ...
        if (clientCertificateStr.empty()) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Cant find matching client certificate.");
        ...
        if (clientPrivateKeyStr.empty()) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Cant find matching client certificate key.");
            return RC_TLS_ENGINE_UNKNOWN_CLIENT_CERTIFICATE_SET_ID;
        }
        ...
        if (wolfSSL_use_certificate_buffer(
            ...
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Error loading client cert.");
            return WolfSSLToEngineError();
        }

        if (wolfSSL_use_PrivateKey_buffer(m_ssl.get(), clientKey, clientPrivateKeyStr.size(), WOLFSSL_FILETYPE_PEM) !=
            WOLFSSL_SUCCESS) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Error loading client key.");
            return WolfSSLToEngineError();
        }
    }
    return RC_TLS_ENGINE_SUCCESSFUL;
...
WolfSSLCertEngine::ocspInit()
{
    if (m_revocationCheckEnabled) {
        if (wolfSSL_EnableOCSPStapling(m_ssl.get()) != WOLFSSL_SUCCESS) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to enable OCSP stapling.");
            return WolfSSLToEngineError();
        }

        if (wolfSSL_UseOCSPStapling(m_ssl.get(), WOLFSSL_CSR_OCSP, 0) != WOLFSSL_SUCCESS) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to use OCSP stapling.");
            return WolfSSLToEngineError();
        }
        ...
        if ((m_cipherSuiteUseCase != CSUSLegacy)) {
            ...
            if (wolfSSL_EnableOCSP(m_ssl.get(), WOLFSSL_OCSP_NO_NONCE | WOLFSSL_OCSP_CHECKALL) != WOLFSSL_SUCCESS) {
                FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to enable online OCSP.");
                return WolfSSLToEngineError();
            }

            if (wolfSSL_SetOCSP_Cb(m_ssl.get(), ocspOnlineCallback, ocspResponseFreeCallback, this) !=
                WOLFSSL_SUCCESS) {
                FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to set online OCSP callback.");
                return WolfSSLToEngineError();
            }

            if (CSUSDefaultWithSoftFail !=m_cipherSuiteUseCase) {
                FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". OCSP fallback mechanism configured to Hard-Fail.");
            }
#else
            if (wolfSSL_CTX_EnableOCSPMustStaple(m_ctx.get()) != WOLFSSL_SUCCESS) {
                FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to enable hard-fail OCSP stapling.");
                return WolfSSLToEngineError();
            }

            FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". OCSP stapling mechanism configured to Hard-Fail.");
#endif

        } else {
            FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". OCSP fallback mechanism configured to Soft-Fail.");
        }
    }
    return RC_TLS_ENGINE_SUCCESSFUL;
}
...
WolfSSLCertEngine::wolfsslConnect()
{
    ...
    {
        FND_LOG_FATAL(GetLogger(), "connectionName: ",m_connectionLoggingName.c_str(),". wolfSSL_check_domain_name failed for domain name: ", m_hostName.c_str());
        return WolfSSLToEngineError();
    }

    if (WOLFSSL_SUCCESS != wolfSSL_connect(m_ssl.get())) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". wolfSSL_connect failed.");
        return WolfSSLToEngineError();
    }
...
WolfSSLCertEngine::calculateCertificatesChainPinningHashes(std::vector<HashSha256>& pinningHashesVec)
{
    ...
    if (nullptr == chain) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Get peer chain failed.");
        return WolfSSLToEngineError();
    }

    int count = wolfSSL_get_chain_count(chain);
    if (count <= 0) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Invalid certificate chain count.");
        return WolfSSLToEngineError();
    }
    ...
    for (int idx = 0; idx < count; ++idx) {
        ...
        if (nullptr == cert) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Get certificate from chain has failed.");
            return WolfSSLToEngineError();
        }
        ...
        if (WOLFSSL_SUCCESS != wolfSSL_X509_get_pubkey_buffer(cert.get(), subjectPublicKeyInfo.data(), &keySize)) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Extracting public key has failed.");
            return WolfSSLToEngineError();
        }
        ...
        if (!calculatePublicKeyPinHash(subjectPublicKeyInfo, hashKeyPinning)) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Calculate certificate pinning failed.");
            return WolfSSLToEngineError();
        }
...
WolfSSLCertEngine::validateHashPinning()
{
    if (m_httpPublicKeyPinningHashes.size() > 0) {
        ...
        if (RC_TLS_ENGINE_SUCCESSFUL != validateHashPinningRC) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to calculate pinning hashed.");
        ...
        if (!atLeastOneCommonMember(actualHashPinning, m_httpPublicKeyPinningHashes)) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Hash pinning failed.");
            return RC_TLS_PUBLIC_KEY_PINNING_FAILED;
        }
    }
...
WolfSSLCertEngine::DoSSLHandshake()
{
    ...
    FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Client Handshake finished.");
...
WolfSSLCertEngine::Shutdown()
{
    ...
    if (m_sslInit) {
        ...
        if (WOLFSSL_SHUTDOWN_NOT_DONE != res)
        {
            ...
            FND_LOG_ERROR(GetLogger(), "wolfSSL_shutdown failed: connectionName: ", m_connectionLoggingName.c_str(), ", wolfssl error code: ", res, ", error message: ", buffer, ".");
        }
...
WolfSSLCertEngine::getUsedProtocol() const
{
    ...
    if (wolfSSL_ALPN_GetProtocol(m_ssl.get(), &protocol_name, &protocol_nameSz) != WOLFSSL_SUCCESS) {
        ...
        FND_LOG_ERROR(GetLogger(), "wolfSSL_ALPN_GetProtocol failed: connectionName: ", m_connectionLoggingName.c_str(), ", wolfssl error code: ", res, ", error message: ", buffer, ".");
        return NONE;
    }
    FND_LOG_VERBOSE(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Received ALPN protocol: ", protocol_name);
...
WolfSSLCertEngine::handleOcspCaching(bool remove)
{
    if (!m_ocspRequestsResponses.empty()) {
        ...
        if (nullptr == ocspHandler) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". OCSP Handler is NULL.");
        ...
        if (requestResponse.isCorrupted()) {
            ...
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Received OCSP Response is corrupted.");
            FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". request ID " , xRequestUniqueId.str());
            return false;
        }
        ...
        if (requestResponse.getIsCached()) {
            if (!remove)
            {
                FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". OCSP Response is already exist in cache. caching is not needed.");
                m_ocspRequestsResponses.clear();
                return true;
            }
            else
        ...
        if (remove)
        {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Revoked OCSP response message.");
            return false;
        }
        ...
        if (nullptr == rawResponseData || 0 == rawResponseSize) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Invalid OCSP response message.");
            FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". response pointer = ", rawResponseData, ", response size = ", rawResponseSize);
            m_ocspRequestsResponses.clear();
            return false;
        }
        ...
        if (nullptr == decodedResponse) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Decoded OCSP Response pointer is NULL.");
            m_ocspRequestsResponses.clear();
            return false;
        }

        if (nullptr == decodedResponse->single || nullptr == decodedResponse->single->status) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Decoded OCSP Response inner pointers are NULL.");
            FND_LOG_DEBUG(GetLogger(),"connectionName: ", m_connectionLoggingName.c_str(), ". decodedResponse->single = ", decodedResponse->single,", decodedResponse->single->status = ", decodedResponse->single->status);
            m_ocspRequestsResponses.clear();
            return false;
        }
...
WolfSSLCertEngine::postVerificationHandler(const WOLFSSL_X509_STORE_CTX* x509StorePtr, bool isFailure)
{
    if (nullptr == x509StorePtr) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". X509 Store context is NULL.");
        return false;
    }

    if (IsHardFailFallbackMechanismActive()) {
        if (nullptr == x509StorePtr->current_cert) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Current certificate pointer is NULL.");
            return false;
        }
            ...
            (wolfSSL_X509_check_issued(x509StorePtr->current_cert, x509StorePtr->current_cert) !=  X509_V_OK))
        {
            if (0 == wolfSSL_X509_ext_isSet_by_NID(x509StorePtr->current_cert, AUTH_INFO_OID)) {
                FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Current certificate does not contain an authInfo extension.");
                return false;
            }
            else
            {
                FND_LOG_INFO(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". authInfo exists");
            }
        }
        else
        {
            FND_LOG_INFO(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". skip authInfo check for root CA");
        }

        if (!handleOcspCaching(isFailure)) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Failed to handle OCSP Response caching.");
            return false;
        }
    }
...
WolfSSLCertEngine::verifyCallback(int preverify, WOLFSSL_X509_STORE_CTX* x509StorePtr)
{
    ...
    FND_LOG_DEBUG(GetLogger(), "Revocation check: STARTED! (preverify = ", preverify, ")");

    bool isFailure = false;
    if (nullptr == x509StorePtr) {
        FND_LOG_FATAL(GetLogger(), "Revocation check: FAILED! X509 store is NULL. The current certificate context was not provided as expected to the verify function. This is an internal wolfSSL FATAL error.");
        FND_LOG_FATAL(GetLogger(), "preverify = ", preverify);
        return WOLFSSL_FAILURE;
    }

    FND_LOG_DEBUG(GetLogger(), "WolfSSL verify result code for the current certificate is ", x509StorePtr->error, ".");

    WOLFSSL* sslCtx =
        (WOLFSSL*)wolfSSL_X509_STORE_CTX_get_ex_data(x509StorePtr, wolfSSL_get_ex_data_X509_STORE_CTX_idx());
    if (nullptr == sslCtx) {
        FND_LOG_FATAL(GetLogger(), "Revocation check: FAILED! WOLFSSL context is NULL. Could not read external data from X509 store. This is an internal wolfSSL FATAL error.");
        FND_LOG_FATAL(GetLogger(), "preverify = ", preverify);
        return WOLFSSL_FAILURE;
    }
    ...
    if (nullptr == wolfCertEngine) {
        FND_LOG_ERROR(GetLogger(), "Revocation check: FAILED! Engine object is NULL. Could not get engine object from the SSL session's context! Make sure the engine object was associated with the SSL session's context (wolfSSL_set_ex_data).");
        FND_LOG_ERROR(GetLogger(), "preverify = ", preverify);
        return WOLFSSL_FAILURE;
    }

    WOLFSSL_X509_CHAIN* chain = wolfSSL_get_peer_chain(sslCtx);
    if (nullptr == chain) {
        FND_LOG_FATAL(GetLogger(), "Revocation check: FAILED! Peer certificate chain is NULL. This is an internal wolfSSL FATAL error.");
        FND_LOG_FATAL(GetLogger(), "preverify = ", preverify);
        return WOLFSSL_FAILURE;
    }
    ...
    int count = wolfSSL_get_chain_count(chain);
    if (count <= 0) {
        FND_LOG_FATAL(GetLogger(), "Revocation check: FAILED! wolfSSL reported an invalid number of certificates in peer chain (count = ", count, "). This is an internal wolfSSL FATAL error.");
        FND_LOG_FATAL(GetLogger(), "preverify = ", preverify);
        return WOLFSSL_FAILURE;
    }

    FND_LOG_DEBUG(GetLogger(), "Number of certificates in peer chain is ", count, ".");
    ...
    if (WOLFSSL_SUCCESS != preverify) {
        ...
        if ((TLSCipherSuiteUseCasesSettings::CSUSDefaultWithSoftFail == wolfCertEngine->GetCipherSuiteUseCase() ||
            ...
            FND_LOG_INFO(GetLogger(), "Revocation check: COMPLETED SUCCESSFULLY! Soft-Fail state. Skip verification of OCSP response.");
        ...
        {
            FND_LOG_INFO(GetLogger(), "Revocation check: COMPLETED SUCCESSFULLY! Skip verification. Will be verified by duplicated certificate.");
            return WOLFSSL_SUCCESS;
        }
    ...
    if (!ret) {
        FND_LOG_ERROR(GetLogger(), "Revocation check: FAILED! Post verification handler failed.");
        FND_LOG_ERROR(GetLogger(), "preverify = ", preverify);
        FND_LOG_ERROR(GetLogger(), "Printing certificate chain in PEM format:");
        for (int i = 0; i < count; i++)
        {
            ...
            if (getCertPemFormatRetVal == WOLFSSL_SUCCESS)
            {
                ...
                FND_LOG_ERROR(GetLogger(), "Certificate index = ", i, ", raw certificate length (Bytes) = ", rawCertificateSize, ", PEM format length (Bytes) = ", certInPemFormatActualSize, ":");
                FND_LOG_ERROR(GetLogger(), std::string(certInPemFormat, certInPemFormat + certInPemFormatActualSize));
            }
        }
        return WOLFSSL_FAILURE;
    }
    if (isFailure) {
        FND_LOG_ERROR(GetLogger(), "Revocation check: FAILED! wolfSSL reported that the current certificate verification has failed.");
        FND_LOG_ERROR(GetLogger(), "preverify = ", preverify);
        FND_LOG_ERROR(GetLogger(), "Printing certificate chain in PEM format:");
        for (int i = 0; i < count; i++)
        {
    ...
    FND_LOG_DEBUG(GetLogger(), "Revocation check: COMPLETED SUCCESSFULLY!");
