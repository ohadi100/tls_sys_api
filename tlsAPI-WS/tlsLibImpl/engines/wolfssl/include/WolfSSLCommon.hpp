...
ServerPSKCallback(WOLFSSL* ssl, const char* identity, unsigned char* key, uint32_t keyMaxLength)
{
    ...
    if (!vwg::tee::TLSTEEAPI::get_instance()->get_psk(pData->remoteHint, pData->hint, &keyData)) {
        FND_LOG_ERROR(GetLogger(), "PSK session key not found");
        return 0;
    }

    if (keyMaxLength < keyData.length) {
        FND_LOG_DEBUG(GetLogger(), "key length mismatch");
        return 0;
    }
...
ocspOnlineCallback(void* ctx, const char* url, int urlSz, unsigned char* req, int reqSz, unsigned char** res)
{
    ...
    FND_LOG_DEBUG(GetLogger(), "Entered ocspOnlineCallback.");
    ...
    if (nullptr == ctx || nullptr == url || nullptr == req || nullptr == res || 0 >= reqSz || 0 >= urlSz ||
        ...
        FND_LOG_ERROR(GetLogger(), "Invalid parameter");
        FND_LOG_DEBUG(GetLogger(), "ctx = 0x%p, url = 0x%p, urlSz = %d, req = 0x%p, reqSz = %d, res = 0x%p");
    ...
    if (nullptr == ocspHandler) {
        FND_LOG_ERROR(GetLogger(), "OCSP Handler is NULL");
        return WOLFSSL_OCSPONLINECB_ERROR;
    }
    ...
    if (std::future_status::ready != futureStatus) {
        FND_LOG_ERROR(GetLogger(), "Timeout reached or the task has not been started yet");
        FND_LOG_DEBUG(GetLogger(), "future status ", (int)futureStatus);
        return WOLFSSL_OCSPONLINECB_ERROR;
    }
    ...
    if (tlsOcspRequestsVector.size() != tlsOcspRequestResponseVector.size()) {
        FND_LOG_ERROR(GetLogger(), "Invalid OCSP responses vector size");
        FND_LOG_DEBUG(GetLogger(), "vector size ", tlsOcspRequestResponseVector.size());
        return WOLFSSL_OCSPONLINECB_ERROR;
    }
    ...
    if (tlsOcspRequestResponse.isCorrupted()) {
        ...
        FND_LOG_ERROR(GetLogger(), "Received OCSP Response is corrupted");
        FND_LOG_DEBUG(GetLogger(), "request ID ", xUniqueId.str(), ", response ID ", xRequestUniqueId.str());
        return WOLFSSL_OCSPONLINECB_ERROR;
    }
    ...
    if (tlsOcspRequestResponse.getRequestUniqueId() != tlsOcspRequest.getUniqueId()) {
        ...
        xUniqueId << std::hex << tlsOcspRequest.getUniqueId();
        xRequestUniqueId << std::hex << tlsOcspRequestResponse.getRequestUniqueId();
        FND_LOG_ERROR(GetLogger(), "Received OCSP response not match to OCSP request");
        FND_LOG_DEBUG(GetLogger(), "request ID ", xUniqueId.str(), ", response ID ", xRequestUniqueId.str());
        return WOLFSSL_OCSPONLINECB_ERROR;
    }
    ...
    if (nullptr == rawOcspResponse || 0 == sizeOfResponse) {
        FND_LOG_ERROR(GetLogger(), "Invalid OCSP response message");
        FND_LOG_DEBUG(GetLogger(),"response pointer = ", rawOcspResponse, ", size = ", sizeOfResponse);
        return WOLFSSL_OCSPONLINECB_ERROR;
    }
    ...
    *res = new (nothrow) unsigned char[sizeOfResponse];
    if (nullptr == *res) {
        FND_LOG_ERROR(GetLogger(), "Failed to allocate buffer for OCSP response message");
        return WOLFSSL_OCSPONLINECB_ERROR;
    }
