...
TLSCertEngine::CheckAuthenticTimeCheck() const
{
    ...
    {
        FND_LOG_VERBOSE(GetLogger(), "Authentic time check: no time check required");
        return RC_TLS_ENGINE_SUCCESSFUL;
    }
    ...
    FND_LOG_DEBUG(GetLogger(), "Authentic time check: STARTED! Started check for connectionName: ", m_connectionLoggingName.c_str(),
                  ". System time: ", currentTimeSec, " [sec], expected time: ", m_checkTime.expectedTime,
                  " [sec], permitted deviation: ", m_checkTime.permittedDeviation, " [sec]");
    ...
    {
        FND_LOG_ERROR(GetLogger(), "Authentic time check: FAILED! The permitted deviation is too big! Permitted deviation: ", absPermittedDeviation, " [sec]");
        return RC_TLS_ENGINE_MAX_PERMITTED_DEVIATION;
    }

    if (-1 == currentTimeSec) { //Attempt to get system time has failed
        FND_LOG_ERROR(GetLogger(), "Authentic time check: FAILED! Could not get system time.");
    ...
    if (nullptr == timeRes) {//Attempt to convert time to local time has failed
        FND_LOG_ERROR(GetLogger(), "Authentic time check: FAILED! Could not convert system time to local time.");
        return RC_TLS_ENGINE_AUTHENTIC_TIMECHECK_FAILED;
    }
    ...
    if (nullptr == timeRes) {//Attempt to convert time to local time has failed
        FND_LOG_ERROR(GetLogger(), "Authentic time check: FAILED! Could not convert expected time to local time.");
        return RC_TLS_ENGINE_AUTHENTIC_TIMECHECK_FAILED;
    }
    ...
    if (currentYear <= (threshold + expectedYear) &&
        ...
        if (abs(diffSec) <= static_cast<double>(absPermittedDeviation)) {
            ...
            FND_LOG_DEBUG(GetLogger(), "Authentic time check: SUCCEEDED! Authentic time check finished successfully! System time: ",
                          currentTimeSec, " [sec], time difference: ", formattedDiffSec.str(), " [sec]");
            return RC_TLS_ENGINE_SUCCESSFUL;
        }
    }

    FND_LOG_ERROR(GetLogger(), "Authentic time check: FAILED! Time difference between system time and expected time is too big!");
    return RC_TLS_ENGINE_AUTHENTIC_TIMECHECK_FAILED;
