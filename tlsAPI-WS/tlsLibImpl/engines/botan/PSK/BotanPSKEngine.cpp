...
Botan::SymmetricKey ClientCredsPSK::psk(const std::string &type,
  ...
  FND_LOG_DEBUG(GetLogger(), "*** CLIENT ***");
...
Botan::SymmetricKey ServerCredsPSK::psk(const std::string &type,
  ...
  FND_LOG_DEBUG(GetLogger(), "*** SERVER ***");
  ...
  while ((remaining > 0) && (false == m_channel->is_closed()) &&
    ...
    if (received < 0) {
      ...
      if (alert != Botan::TLS::Alert::NULL_ALERT) {
        if (m_isServer) {
          FND_LOG_INFO(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Server alert: ", alert);
        } else {
...
TLSEngineError BotanPSKEngine::doSSLHandshakeServer() {
  ...
  while (false == m_channel->is_active()) {
    if (true == m_channel->is_closed()) {
      FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". doSSLHandshakeServer failed - m_channel closed");
      res = AlertToEngineError(m_receivedAlert);
      break;
    }
    ...
    res = feed();
    if (res != RC_TLS_ENGINE_SUCCESSFUL) {
      FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". feed failed (res: ", res, ")" );
      break;
    }
  }
...
TLSEngineError BotanPSKEngine::DropTLS() {
    if (!m_dropSendStarted) {
        try {
            ...
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". DropTLS failed");
        }
