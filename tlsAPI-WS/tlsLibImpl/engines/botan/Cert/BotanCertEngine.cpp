    ...
    if (!m_engine->m_privateStore) {
        if (!serverCert.empty()) {
            try {
                ...
                FND_LOG_ERROR(GetLogger(), e.what());
            }
        }
    }
                                     ...
                                     const Botan::TLS::Policy&                                        policy)
{
    ...
    if (m_engine->IsHardFailFallbackMechanismActive()) {
        ...
        FND_LOG_DEBUG(GetLogger(), "OCSP fallback mechanism configured to Hard-Fail");
...
#else
        FND_LOG_DEBUG(GetLogger(), "OCSP stapling mechanism configured to Hard-Fail");
        if (ocsp.empty()) {
            throw Botan::Exception("Certificate validation failure: No OCSP stapling from server");
        }
#endif
    } else {
        FND_LOG_DEBUG(GetLogger(), "OCSP fallback mechanism configured to Soft-Fail");
    }
...
CallbacksCert::tls_alert(Botan::TLS::Alert alert)
{
    ...
    FND_LOG_ERROR(GetLogger(), "alert: ", alert.type_string().c_str());
...
BotanCertEngine::setCipherSuitesListUseCase(const TLSCipherSuiteUseCasesSettings& cipherSuiteSettings)
{
    ...
    case CSUSLegacy:
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". CSUSLegacy cipher suite list.");
    ...
    case CSUSLongtermSecure:
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". CSUSLongtermSecure cipher suite list.");
    ...
    case CSUSIanaRecommended:
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". CSUSIanaRecommended cipher suite list.");
        m_ciphersuiteCodes   = ianaRecommendedCipherSuites;
    ...
    case CSUSDefault:
    default:
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". CSUSDefault cipher suite list.");
        m_ciphersuiteCodes   = defaultCipherSuites;
    ...
    while ((remaining > 0) && (false == m_client->is_closed()) && (RC_TLS_ENGINE_SUCCESSFUL == res)) {
        ...
        if (received < 0) {
            ...
            if (true == m_client->is_closed()) {
                FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". CHANNEL CLOSED.");
            } else {
                try {
            ...
            if (alert != Botan::TLS::Alert::NULL_ALERT) {
                FND_LOG_INFO(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Client alert: ", alert, ".");
...
BotanCertEngine::checkTeeAndItsData()
{
    ...
    if (!m_tlsTeeApi) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Fatal error: can't access TEE.");
        return RC_TLS_ENGINE_TEE_ACCESS_ERROR;
    }

    string serverCert = m_tlsTeeApi->get_root_cert_bundle(m_certStoreId);
    if (serverCert.empty()) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Fatal error: can't find root cert bundle.");
    ...
    if (!m_clientCertificateSetID.empty()) {
        ...
        if (clientCertificateStr.empty()) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Fatal error: can't find client cert");
        ...
        if (clientPrivateKeyStr.empty()) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Fatal error: can't find client cert key");
            return RC_TLS_ENGINE_UNKNOWN_CLIENT_CERTIFICATE_SET_ID;
        }
    }
...
BotanCertEngine::DoSSLHandshake()
{
    ...
    while (false == m_client->is_active()) {
        if (true == m_client->is_closed()) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". SSL Handshake Client failed - the channel closed");
        ...
        if (res != RC_TLS_ENGINE_SUCCESSFUL) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". feed failed");
            break;
        }
    }
    FND_LOG_INFO(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". SSL Handshake Client finished");
...
BotanCertEngine::Shutdown()
{
    try {
        ...
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Shutdown engine failed");
    }
    return RC_TLS_ENGINE_SUCCESSFUL;
}
...
BotanCertEngine::getUsedProtocol() const
{
    ...
    FND_LOG_VERBOSE(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Received ALPN protocol: ", protocol_name.c_str());
                                              ...
                                              std::vector<TLSOcspCachedResponse>& outTlsOcspCachedResponses) const
{
    if (ocspRequestsCertsTupleVector.size() != tlsOcspResponses.size()) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Invalid OCSP responses vector size");
        FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(),
    ...
    for (size_t i = 0; i < tlsOcspResponses.size(); i++) {
        ...
        if (tlsOcspResponse.isCorrupted()) {
            ...
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Received corrupted OCSP response");
            FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". response ID ",xReceivedResponseUniqueId.str());
            isErrorRaised = true;
            continue;
        }
        ...
        if (requestUniqueId != receivedResponseUniqueId) {
            ...
            xReceivedResponseUniqueId << std::hex << receivedResponseUniqueId;
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Received OCSP response not match to OCSP request");
            FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". request ID ", xRequestUniqueId.str(), ", response ID ", xReceivedResponseUniqueId.str());
            isErrorRaised = true;
            continue;
        }
        ...
        if (Certificate_Status_Code::OCSP_SIGNATURE_OK != certStatus) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". OCSP Response check signature failure");
            FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Certificate status code is ", to_string(certStatus));
            //Delete from cache!
            isErrorRaised = true;
            continue;
        }
        ...
        if (Certificate_Status_Code::OCSP_RESPONSE_GOOD != certStatus) {
            FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Bad certificate status");
            FND_LOG_DEBUG(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Certificate status code is ", to_string(certStatus));
            //Delete from cache!
            isErrorRaised = true;
            continue;
                                 ...
                                 const std::vector<std::shared_ptr<const Botan::OCSP::Response>>& ocsp) const
{
    ...
    if (1 >= certChain.size()) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Root certificate isn't included or empty chain");
        return false;
    }

    if (certChain.size() != certificatePathStatusCodes.size()) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Cert chain and Cert status codes size mismatch");
        return false;
    }
    ...
    if (std::future_status::ready != futureStatus) {
        FND_LOG_ERROR(GetLogger(), "connectionName: ", m_connectionLoggingName.c_str(), ". Timeout reached or the task has not been started yet");
        return false;
    }
