...
InternIOStream::InternIOStream(int fd)
  ...
  , m_isBlocking(true)
{
    FND_LOG_INFO(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". new stream created from fd: ", fd);
}
  ...
  : m_port(port)
{
    ...
    if (setsockopt(m_fd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". setsockopt(SO_REUSEADDR) failed");
    }
}
...
InternIOStream::receive(void* buf, uint32_t len)
{
    ...
    if (0 > m_fd || nullptr == buf) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Error receive on socket");
...
InternIOStream::send(const void* buf, uint32_t length)
{
    if (0 > m_fd || nullptr == buf || 0 >= length) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Error send on socket");
        return RC_STREAM_IO_ERROR;
    }
    ...
    while (!m_dataToSend.empty()) {
        ...
        if (actualLength < 0) {
            if (!m_isBlocking) {
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    FND_LOG_DEBUG(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(),
                                  ". Can't send now, it will be sent in the future (blocking: ", (m_isBlocking) ? "true" : "false", ")");
                    return length;  // the data to sent was already stored in the buffer
                }
            }
            FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Can't send - IO error ",
        ...
        if ((size_t)actualLength < size) {
            FND_LOG_DEBUG(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(),
                          ". Sent only part of the data (", actualLength, "/", size, "), will resume later on");
        ...
        if ((size_t)actualLength == size) {
            ...
            FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(),
                          ". Weird error - sent more than buffer (", actualLength, "/", size, ")");
            return RC_STREAM_IO_ERROR;
        }
    }
...
InternIOStream::close()
{
    if (m_fd > 0) {
        ...
        if (fcntl(m_fd, F_GETFD) != -1 || errno != EBADF) {
            if (-1 == ::close(m_fd)) {
                FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(),
                              ". Unable to close socket: ", m_fd, " - ", strerror(errno));
            } else {
                FND_LOG_DEBUG(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Closed socket: ", m_fd);
            }
            m_fd = -1;
        }
    }
...
{
    if (m_isOpen) {
        FND_LOG_INFO(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Connection already established on socket");
        return false;
    }

    if (0 > m_fd) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Trying to connect a closed socket");
        return false;
    }

    if (m_ipAddress.empty()) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". IP address is undefined. Unable to connect");
        return false;
    }
    if (-1 == ::connect(m_fd, m_addr, m_addrSize)) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(),
                      ". Unable to connect to ", m_ipAddress.c_str(), "(", m_port, ") - ", strerror(errno));
        close();
        return false;
    }
...
InternIOStream::Listen()
{
    ...
    if (0 > m_fd) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Trying to listen on closed socket");
        return false;
    }

    if (m_ipAddress.empty()) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Unable to listen on undefined ipAddress");
        return false;
    }

    FND_LOG_DEBUG(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Binding socket: ", m_fd);
    if (-1 == ::bind(m_fd, m_addr, m_addrSize)) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Unable to bind to: ",
                      m_ipAddress.c_str(), "(", m_port, ") - err: ", strerror(errno));
        close();
        return false;
    }

    FND_LOG_DEBUG(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Listening on socket");
    /*
     * was 0, changed to 1 due to ICAS1 qemu issues.
     */
    if (-1 == ::listen(m_fd, 1)) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Unable to listen on socket (err: ", strerror(errno), ")");
        close();
        return false;
    }
...
{
    FND_LOG_VERBOSE(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Accept started on stream");
    if (0 > m_fd) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Trying to accept on closed socket");
        return nullptr;
    }
    ...
    FND_LOG_DEBUG(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Calling accept on socket: ", m_fd);
    int workingSock = ::accept(m_fd, nullptr, nullptr);
    if (-1 == workingSock) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(),
                      ". Unable to accept on socket ", workingSock, " (err: ", strerror(errno), ")");
        close();
        return nullptr;
    }

    FND_LOG_DEBUG(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". New socket connected: ", workingSock);
...
InternIOStream::GetLocalAddress() const
{
    ...
    if (res.failed()) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Create IPAddress failed");
    }
...
{
    if (m_fd < 0) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Socket undefined - unable to set to non-blocking (fd: ", m_fd, ")");
        return false;
    }
...
{
    if (timeout < 0) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". Timeout cannot be negative");
        return;
    }
    ...
    if (setsockopt(m_fd, SOL_SOCKET, SO_RCVTIMEO, (struct timeval*)&tv, sizeof(struct timeval))) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". setsockopt(SO_RCVTIMEO) failed");
    }

    if (setsockopt(m_fd, SOL_SOCKET, SO_SNDTIMEO, (struct timeval*)&tv, sizeof(struct timeval))) {
        FND_LOG_ERROR(GetLogger(), "connectionName = ", getConnectionLoggingName().c_str(), ". setsockopt(SO_SNDTIMEO) failed: ", std::to_string(errno).c_str());
    }
}
