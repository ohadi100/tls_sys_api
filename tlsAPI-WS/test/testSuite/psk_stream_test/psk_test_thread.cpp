...
void taskSndRcv(std::shared_ptr<vwg::tls::ITLSSessionEndpoint> sessionEP, uint loop, const std::string &msgSend)
{
    ...
    while (loop)
    {
        ...
        // *** receive ***
        FND_LOG_INFO(GetLogger(), "------------------ start receive --------------" );
        Int32 receivedLen = sessionEP->receive(buffer, sizeof(buffer));
        FND_LOG_INFO(GetLogger(), "------------------ end receive --------------" );

        if (receivedLen < 0)
        {
            auto err = sessionEP->getPendingErrors();
            FND_LOG_ERROR(GetLogger(), "CLIENT error receiving data from session: ", err);
            break;
        }
...
void taskClose(std::shared_ptr<vwg::tls::ITLSSessionEndpoint> sessionEP)
{
    ...
    FND_LOG_INFO(GetLogger(), "------------------ start shutdown --------------" );
    sessionEP->shutdown();
    FND_LOG_INFO(GetLogger(), "------------------ end shutdown --------------" );
}
                   ...
                   const std::string &msgSend, const uint16_t loop)
{
    ...
    vwg::tls::TLSClientSocketResult Socket_rc = socketFactory_rc.getPayload()->createClientSocket(ipAddress, port,
                                                                                                  ...
                                                                                                  confidentiality);
    if (Socket_rc.failed()) {
        FND_LOG_ERROR(GetLogger(), "error creating client socket: ", Socket_rc.getErrorCode());
        return;
    }
    ...
    if(t2.joinable())
    {
        FND_LOG_INFO(GetLogger(), "start 2.join();" );
        t2.join();
        FND_LOG_INFO(GetLogger(), "end 2.join();" );
    }
    if(t1.joinable())
    {
        FND_LOG_INFO(GetLogger(), "start 1.join();" );
        t1.join();
        FND_LOG_INFO(GetLogger(), "end 1.join();" );
    }
}
