    ...
    std::async(std::launch::async, [&]() -> void {
        for (const TLSOcspCachedResponse& response : responses) {
            ...
            if (response.getResponse().empty()) {
                FND_LOG_DEBUG(GetLogger(),"received empty response caching request with key ", xRequestUniqueId.str(), ", deleting from cache");
                mCachedResponses.erase(response.getRequestUniqueId());
            }
            else {
                FND_LOG_DEBUG(GetLogger(),"Caching response with key ", xRequestUniqueId.str());
    ...
    return std::async(std::launch::async, [&]() -> std::vector<TLSOcspRequestResponse> {
        ...
        try {
            for (const TLSOcspRequest& request : requests) {
                ...
                FND_LOG_DEBUG(GetLogger(),"Processing request with key ", xUniqueId.str());
                ...
                if (mCachedResponses.end() != responseFromCacheRes) {
                    ...
                    FND_LOG_DEBUG(GetLogger(),"Found response in cache with key ", xUniqueId.str());
                ...
                if (!decodeUrl(request.getRequestUrl(), ocspResponderDomainName, ocspResponderPort)) {
                    FND_LOG_ERROR(GetLogger(), "Failed to decode OCSP responder's URL");
                ...
                if (!createTcpConnection(sockFd, ocspResponderDomainName, ocspResponderPort)) {
                    FND_LOG_ERROR(GetLogger(), "Failed to create TCP connection to OCSP responder");
                    responses.push_back(TLSOcspRequestResponse(request.getUniqueId()));
                    continue;
                }
                ...
                if (!sendMessage(sockFd, httpOcspRequestMessage)) {
                    FND_LOG_ERROR(GetLogger(), "Failed to send HTTP POST message to OCSP responder");
                    responses.push_back(TLSOcspRequestResponse(request.getUniqueId()));
                    continue;
                }

                if (!processHttpOcspResponse(sockFd, request, responses)) {
                    FND_LOG_ERROR(GetLogger(), "Failed to process received OCSP response");
                    responses.push_back(TLSOcspRequestResponse(request.getUniqueId()));
                    continue;
                }
            }
            ...
            FND_LOG_ERROR(GetLogger(), "Unexpected exception has been raised.");
