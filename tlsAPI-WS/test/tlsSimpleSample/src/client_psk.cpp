...
int main()
{
    ...
    {
        FND_LOG_ERROR(GetLogger(), "error creating client socket: ", clientSocket_rc.getErrorCode());
    ...
    FND_LOG_INFO(GetLogger(), "connected to server");

    FND_LOG_DEBUG(GetLogger(), "Trying to read: \"Hello, Client!\"");
    uint8_t buffer[1024];
    ...
    FND_LOG_DEBUG(GetLogger(), "Sending: \"Hello, Server!\"");
