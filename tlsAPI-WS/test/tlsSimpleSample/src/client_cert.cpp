...
int main (int argc, char *argv[])
{
    if(argc != NUM_OF_PARAMS) {
        FND_LOG_ERROR(GetLogger(), "argc error");
        return 0;
    }
    ...
    if (!ptr->Connect())
    {
        FND_LOG_ERROR(GetLogger(), "Stream connected failed");
        return -1;
    }
    ...
    TLSClientSocketResult clientSocket_rc =
            socketFactory_rc.getPayload()->createTlsClient(ptr, hostName,
                    certStoreId, clientCertificateSetID, cipherSuiteIds, checkTime, httpPublicKeyPinningHashs);
