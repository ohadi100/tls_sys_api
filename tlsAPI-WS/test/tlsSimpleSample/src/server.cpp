...
int main()
{
    ...
    {
        FND_LOG_ERROR(GetLogger(), "error creating client socket: ", serverSocket_rc.getErrorCode());
        return serverSocket_rc.getErrorCode();
    }



    FND_LOG_INFO(GetLogger(), "waiting for client to connect");
    TLSResult<std::shared_ptr<ITLSSessionEndpoint>> session_ep_rc = serverSocket_rc.getPayload()->accept();
    if (session_ep_rc.failed())
    {
        FND_LOG_ERROR(GetLogger(), "error accepting connection: ", session_ep_rc.getErrorCode());
        return session_ep_rc.getErrorCode();
    }
    FND_LOG_INFO(GetLogger(), "received new connection from client");

    std::shared_ptr<ITLSSessionEndpoint> sessionEP = session_ep_rc.getPayload();

    FND_LOG_INFO(GetLogger(), "Sending: \"Hello, Client!\"");
    ...
    FND_LOG_INFO(GetLogger(), "Trying to read: \"Hello, Server!\"" );
    uint8_t buffer[1024] = {0};
    Int32 receivedLen = sessionEP->receive(buffer, sizeof(buffer));
    if (receivedLen < 0)
    {
        FND_LOG_ERROR(GetLogger(), "Error receiving data from session");
        return receivedLen;
    }
    if (receivedLen != 15)
    {
        FND_LOG_ERROR(GetLogger(), "received size mismatched");
        return 1;
    }
    //FND_LOG_DEBUG(GetLogger(), "buffer read: ", buffer);
