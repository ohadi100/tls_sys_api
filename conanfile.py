...
class Recipe(ConanFile):
    ...
    def build(self):
        cmake = CMake(self)
        environ["FND_LOG_DEV_DIR"] = self.deps_cpp_info["fnd_log"].rootpath
        environ["SYSAPI_LOG_DEV_DIR"] = self.deps_cpp_info["sysapi_log"].rootpath
        environ["SYSAPI_PER_DEV_DIR"] = self.deps_cpp_info["sysapi_per"].rootpath
        if self.settings.os == "Neutrino":
            environ["CC"] = "qcc"
    ...
    def package(self):
        ...
        cmake.configure()
        cmake.install()
        self.copy("*", "include", "tlsAPI/includes")
        self.copy("*.so*", "lib/sysapi", "third_party/install/lib")
        self.copy("libtls_wolfssl.so", "lib/sysapi", "../build/lib")
        symlink("libtls_wolfssl.so", f"{self.package_folder}/lib/sysapi/lib{self.name}.so")
